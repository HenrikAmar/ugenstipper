...
// Samme "udvid"-mønster som miniligaerne (se MiniligaStanding.tsx) - viser
// som udgangspunkt kun toppen (+ din egen placering, hvis du ikke selv er
// med i toppen), men lader dig folde hele stillingen ud med et tryk.
export function StandingList({
  ranking,
  userId,
  windowSize = 5,
  expandLabel = "Udvid stillingen",
}: {
  ranking: RankRow[];
  ...
  const display = expanded
    ...
    : buildStickyRanking(ranking, userId, windowSize);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {ranking.length > windowSize && (
        <button
          type="button"
          onClick={() => setExpanded((v) => !v)}
          className="mt-1 w-full text-center text-[12.5px] font-semibold text-accent underline"
        >
          {expanded ? `Vis kun top ${windowSize}` : expandLabel}
        </button>
      )}
    </div>
  );
